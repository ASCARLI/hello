...
public:
    ...
    iterator olderase(iterator from, iterator to)
    {
    ...
    iterator erase(iterator from, iterator to)
    {
        ...
        int cnt = 0;
        Node *p_prev, *p_next;
        p_prev = p_next = from.current->next;
        while (p_next != to.current)
        {
            p_next = p_next->next;
            delete p_prev;
            ++cnt;
        ...
        from.current->next = to.current;
        to.current->prev = from.current;
        return to;
    }
    ...
    iterator find(Object ele) {
        ...
        std::cout << ele <<"is not founded." << std::endl;
        return nullptr;
    }
